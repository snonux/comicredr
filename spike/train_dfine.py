#!/usr/bin/env python3
"""Fine-tune D-FINE (Apache-2.0, COCO-pretrained) on our labels, on CPU.

  python3 spike/train_dfine.py PAGES_DIR [PAGES_DIR ...] --out spike/out/dfine \
      [--weights ustc-community/dfine-small-coco] [--epochs 30] [--imgsz 640]
  python3 spike/train_dfine.py --export spike/out/dfine/last --out-onnx dfine.onnx

A clean-licence alternative to spike/train.py: no Ultralytics code or
weights (AGPL) and no Manga109 data. The weights start from D-FINE's
COCO-only checkpoint (Hugging Face transformers port), never the
Objects365 ones, whose data is academic-use only.

Same labels, same held-out split and the same three classes as
spike/train.py (frame, caption, balloon), so spike/evaluate.py compares the
two directly. --export writes an ONNX graph with the app's interface: one
float32 image [1, 3, 800, 800] (RGB 0..1, letterboxed to the top-left, grey
114 padding) in, [1, 300, 6] out (x0, y0, x1, y1 in input pixels, score,
class). The graph resizes to the model's own size inside, so the app needs
no change. Needs `transformers` (>= 4.52), torch and onnxslim.
"""
import argparse
import math
import random
...
from train import CLASSES, build_dataset  # noqa: E402
...
def train(a):
    ...
    for epoch in range(a.epochs):
        ...
        ema.module.save_pretrained(out / "last")
        if vloss < best:
            best = vloss
            ema.module.save_pretrained(out / "best")
...
def export(a):
    ...
    graph = AppGraph(model, a.imgsz).eval()
    ...
    for k, v in {"names": str(dict(enumerate(CLASSES))), "imgsz": f"[{APP_INPUT}, {APP_INPUT}]",
                 "description": f"D-FINE fine-tune ({a.imgsz} px inside), COCO init, Apache-2.0"}.items():
...
def main():
    ...
    ap.add_argument("--out", default="spike/out/dfine")
